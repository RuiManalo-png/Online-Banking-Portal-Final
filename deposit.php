...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate amount
    if ($amount <= 0) {
        echo "<script>alert('Invalid deposit amount.'); window.history.back();</script>";
        exit;
    }
    ...
    // Begin transaction for deposit
    mysqli_begin_transaction($conn);
    
    try {
        $deposit_query = "UPDATE accounts SET balance = balance + '$amount' WHERE id = '$account_id'";
        mysqli_query($conn, $deposit_query);

        // Record transaction
        $transaction_query = "INSERT INTO transactions (account_id, type, amount, description) 
                            VALUES ('$account_id', 'Deposit', '$amount', 'Deposit made')";
        ...
        $modalType = "error";
        $modalMessage = "Deposit failed. Please try again.";
